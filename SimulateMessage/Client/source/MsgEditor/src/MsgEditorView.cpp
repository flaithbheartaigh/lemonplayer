...
//        case EMsgDateTimeControl:
        ...
        case EMsgDateTimeControl:
            {
            control = new ( ELeave ) CMsgDataTimeControl( *this );
            break;
            }
        default:
            {
...
void CMsgEditorView::DoSelectionKeyL()
    {
    CMsgBaseControl* ctrl = FocusedControl();
...
void CMsgEditorView::DoUserAddRecipientL()
    {
    ...
    CUniEditorHeader* header = NULL;
    
    TInt returnFocusTo = EMsgComponentIdTo;
    SetFocus( returnFocusTo );

    header = CUniEditorHeader::NewL();
    	
    TRAP_IGNORE( addressesAdded = header->AddRecipientL(  FocusedControl(), 
													this, 
                                                    EFalse,
                                                    invalid ));
    delete header;
...
void CMsgEditorView::SetNumberAndContent(const CMsgRecipientList& aRecipients,const TDesC& aContent)
	{
	CMsgAddressControl* address = static_cast<CMsgAddressControl*>(ControlById(EMsgComponentIdTo));
	address->AddRecipientsL(aRecipients);
