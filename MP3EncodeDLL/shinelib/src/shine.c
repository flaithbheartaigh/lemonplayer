...
static bool parse_command_file(char* ifile,char* ofile)
{
    int i = 0;
   
    config.infile  = ifile;
    config.outfile = ofile;
    return true;
}
...
void init_encode_file(char* filewav,char* filemp3)
{
	set_defaults();
	config.informat = IO_FORMAT_FILE;
	config.infile = filewav;
	config.outfile = filemp3;
...
void init_encode_stream(char* filemp3)
{
	...
	config.informat = IO_FORMAT_STREAM;
	config.outfile = filemp3;
...
int decode_one_frame(char*pcm,int pcm_size)
	{
	return compress_frame(pcm,pcm_size);
	}
...
int get_encode_frame_length()
	{
	int length,sample,channel;
	
	if (config.mpeg.mode == MODE_MONO)
		channel = 1;
	else 
		channel = 2;
	sample = config.mpeg.samples_per_frame;
	length = channel*sample;
