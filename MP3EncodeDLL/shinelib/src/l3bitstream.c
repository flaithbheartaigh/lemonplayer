...
void close_frame_bitstream()
	{
	int gr, ch;
	if ( PartHoldersInitialized )
	    {
	    BF_freePartHolder(headerPH);
		BF_freePartHolder(frameSIPH);

		for ( ch = 0; ch < MAX_CHANNELS; ch++ )
			BF_freePartHolder(channelSIPH[ch]);

		for ( gr = 0; gr < MAX_GRANULES; gr++ )	
		    for ( ch = 0; ch < MAX_CHANNELS; ch++ )
		    {
		    BF_freePartHolder(spectrumSIPH[gr][ch]);
		    BF_freePartHolder(scaleFactorsPH[gr][ch]);
		    BF_freePartHolder(codedDataPH[gr][ch]);
		    BF_freePartHolder(userSpectrumPH[gr][ch]);
		    }
		BF_freePartHolder(userFrameDataPH);
