...
static int read_samples_stream(short *sample_buffer,
                        int    frame_size,int samples_read)
{
...
int wave_get_stream(short buffer[2][samp_per_frame],char* data,int size)
/* expects an interleaved 16bit pcm stream from read_samples, which it */
/* de-interleaves into buffer                                          */
{
    short 		*temp_buf;
    ...
    {
        case MODE_MONO  :
            samples_read = read_samples_stream(temp_buf,
            		config.mpeg.samples_per_frame,size);
            for(j=0;j<samp_per_frame;j++)
            {
                buffer[0][j] = temp_buf[j];
                buffer[1][j] = 0;
		...
		default:
            samples_read = read_samples_stream(temp_buf,
            		config.mpeg.samples_per_frame<<1,size);
            for(j=0;j<samp_per_frame;j++)
            {
                buffer[0][j] = temp_buf[2*j];
                buffer[1][j] = temp_buf[2*j+1];
