...
static void free_side_info_data( side_info_link *l )
{
	int ch, gr;
	
	BF_freePartHolder(l->side_info.headerPH);
...
{
	side_info_link *f1 = l;
    side_info_link *f2 = NULL;

    while (f1)
    {
    	f2 = f1;
    	f1 = f1->next;
    	free_side_info_data(f2);
    }
}

int release_side_info()
	{
	free_side_info_link(side_queue_head);
	free_side_info_link(side_queue_free);
	}
